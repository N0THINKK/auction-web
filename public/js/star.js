...
class NightSky {
  constructor() {
    ...
    this.numberOfStars = Number.parseInt(
      this.nightSky.style.getPropertyValue("--number-of-stars")
    );
...
function adjustNightSkyHeight() {
  const nightSky = document.getElementById("night-sky");
  if (nightSky) {
    const bodyHeight = document.body.scrollHeight;
    nightSky.style.height = `${bodyHeight}px`;
  }
}

document.addEventListener("DOMContentLoaded", adjustNightSkyHeight);
window.addEventListener("resize", adjustNightSkyHeight);
window.addEventListener("scroll", adjustNightSkyHeight);
